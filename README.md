...
A free, **BSD-3-licensed** one-page theme for **Tiger** (TigerZF). Self-contained — Bootstrap 5,
Font Awesome, and fonts are all vendored, **no external CDN dependencies**. Ported to Tiger's
theme-module model; third-party heritage is recorded in [NOTICE.md](NOTICE.md).
...
Everything is committed (BSD-3), so a `git clone` is a complete, renderable theme — no fetch step.
